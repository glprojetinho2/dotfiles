...
paru -S --needed --noconfirm socat yq wf-recorder rofi python-pip python python-pipx python-i3ipc anki-bin syncthing ripgrep swaybg imv alacritty lldb rustup git-credential-oauth pipewire pipewire-pulse pwvucontrol lazygit tealdeer kdotool yazi bat polkit multibg-sway waybar sway jq wl-clipboard swaylock grim slurp zoxide fish fisher torbrowser-launcher tor qutebrowser calibre  helix starship glow links pup-bin aha || { echo "falha ao instalar pacotes essenciais"; exit 1; }
pipx install jrnl
tldr --update
